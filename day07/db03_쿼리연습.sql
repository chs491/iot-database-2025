...
/* 문제1 - employee에서 사원의 성과 이름을 Name, 업무는 job, 급여는 Salary, 연봉에 $100 보너스를 추가해서 계산한 Increased Ann_Salary
			급여에 $100를 추가해서 Increased Salary 별칭으로 출력하시오.(107행)
...
select concat(first_name, ' ', last_name) as 'Name' 
 ...
 /* 문제2 - Employee에서 모든 사원의 last_name과 연봉을 '이름 : 1 Year Salary = $연봉'형식으로 출력하고, 1 Year Salary라는 별칭으로 붙이세요(107행)
 */ 
...
/* 샘플 - hr부서 예산 편성 문제로 급여 정보 보고서를 작성한다. employees에서 salary가 7000 ~ 10000달러 범위 이외의 사람의
			성과 이름을 Name해서 급여를 급여가 작은 순서로 출력하시오(75행)
*/   
select concat(first_name, ' ', last_name) as 'name'
       , salary
  from employees
 ...
 /* 문제1 - 사원의 last_name 중에서 e나 o 글자를 포함한 사원을 출력하시오. 이때 컬럼명은 e And o Name이라고 출력하시오.(10행)
 */
 select last_name as 'e And o Name'
   from employees
  where last_name like '%e%' and last_name like '%o%';
  
/* 문제2 - 현재 날짜 타입을 날짜함수를통해, 1995년 5월 20일 ~ 1996년 5월 20일 사이 입사한 사원의 이름 Name으로 별칭
...
/* 문제3 - 사원의 성과 이름을 Name으로 별칭, 입사일, 입사한 요일을 출력하시오. 이때 주(week) 시작인 일요일부터 출력되도록 하시오.(107행)
*/ 
select concat(first_name, ' ', last_name) as 'name'
	 , hire_date
...
select concat(e.first_name, ' ', e.last_name) as 'name'
     ...
     , (select grade_level from job_grades
		 where e.salary between lowest_sal and highest_sal) as 'grad_level' -- 서브쿼리 추가!!
