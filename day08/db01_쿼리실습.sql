...
-- 서브쿼리 계속
/* 문제1 - 사원의 급여 정보 중 업무별(job) 최소 급여를 받는 사원의 이름, 성을 name으로 별칭, 업무, 급여, 일사일로 출력(21행)
*/
desc jobs;
select * from jobs;
...
-- 집합연산자 : 테이블 내용을 합쳐서 조회하는 것

-- 조건부 논리 표현식 제어 : CASE -> if문이랑 동일
/* 샘플 문제1 - 프로젝트 성공으로 급여 인상이 결정됨.
...
/* 문제3 - 월별로 입사한 사원수가 아래와 같이 출력되도록 하시오. 12행
*/  
-- 형변환 함수 cast(), convert()
...
-- ROLLUP
/* 샘플 - 부서와 업무별 급여 합계를 구하고 신년도 급여수준 레벨을 지정하려고 함.
	부서번호와 업무를 기준으로 정해 행을 그룹별로 나누어 급여합계와 인원수를 출력(20개행)
*/
...
/* 문제1 - 이전문제를 활용, 집계결과가 아니면(ALL-DEPTs)라고 출력하고, 업무에 대한 집계결과가 아니면(ALL-JOBs)를 출력
...
/* 문제1 - 부서별 급여를 기준으로 내림차순 결정하시오. 이때 다음 세가지 함수를 이용하여, 순위를 출력하시오(107행)
*/
select employee_id
	 , last_name
     , salary
...
select employee_id
     ...
     , dense_rank() over (partition by department_id order by salary desc) as 'Dense Rank'-- 1,1,2 순위매기기 rank
