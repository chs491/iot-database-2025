...
SELECT bookname AS '제목'
        ...
        , LENGTH(bookname) AS '제목 바이트수' -- utf-8에서 한글 한글자의 바이트수는 3bytes 
  FROM Book
 ...
 select SUBSTR('성유고', 2,3); -- 연습이 필요
 
 SELECT substr(name, 1,1) AS '성씨구분'
		, count(*) AS '인원수'
   FROM Customer
...
SELECT ADDDATE(SYSDATE(), INTERVAL 9 HOUR) AS '현재한국시간';
...
-- NULL = Python None 동일. 모든 다른 프로그래밍 언어에서는 전부 NULL, NULMyBook
