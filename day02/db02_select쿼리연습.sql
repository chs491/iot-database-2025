...
-- 3-2 : 모든 도서의 도서번호, 도서이름, 출판사, 가격을 입력하시오
-- 파이썬, C# 등 프로그램이언어로 가져갈 때는 컴럼이름, 컬럼개수를 모두 파악해야 하기 때문에 아래와 같이 사용
...
-- < 미만, > 초과, <=이하 >= 이상 <> 같지 않다 != 같지 않다 = 같다(프로그래밍언어와 차이)
SELECT *
  FROM Book
 WHERE price < 20000; 
...
-- % 갯수와 상관없이 글자가 포함되는 것
-- []은 MySQL에서는 사용불가
-- [0-5] 1개의 문자가 일치. 한글자가 0에서 5의 숫자를 포함한다. 
-- [^] 1개의 문자가 일치하지 않는 것
-- _ 특정위치의 1개의 문자가 일치할 때 _구% 첫번째 글자는 뭐든지 상관없고, 두번쨰 글자가 '구'로되고 뒤에 글이 더 있다.
SELECT *
  FROM Book
