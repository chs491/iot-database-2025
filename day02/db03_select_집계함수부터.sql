...
-- MySQL 문자열은 '' 홑따옴표 사용, "" 쌍따옴표는 사용 불가!!alter

-- 3-16 : 김연아 고객이 주문한 도서의 총판매액을 구하시오.
SELECT SUM(saleprice) as 총매출 
  FROM Orders
 WHERE custid = 2; 
...
-- GROUP BY를 사용하면 반드시 집계함수 및 GROUP BY에 포함된 컬럼이 SELECT 안에 포함시켜야 한다.
-- GROUP BY에 있는 컬럼만 SELECT 안에 사용할 수 있다. 그 외는 사용불가 
SELECT custid
...
-- COUNT() 등 집계함수는 WHERE에 넣을 수 없다.
SELECT custid
	, COUNT(*) AS 총수량
  FROM Orders
 WHERE saleprice >= 8000
 GROUP BY custid
 HAVING COUNT(*) >= 2; -- COUNT(*)와 별명을 사용할 수 있다. 
...
-- 3-30은 조인으로 변경 가능(SubQuery <--> Join)
SELECT c.name
...
-- 3-31 : 출판사별로 출판사의 평균도서가겨격보다 비싼 도서를 구하시오
-- 상관 서브쿼리
SELECT *
  FROM Book AS b1
 WHERE  b1.price > ( 
