...
def showDatas():
    ...
    data = cursor.fetchall() # 쿼리 실행 데이터를 전부 가져옴
...
def getData(event):
    '''
    트리뷰 더블클릭으로 선택된 학생 정보를 엔트리 위젯에 채우는 사용자함수
...
def addData():
    '''
    새 학생정보 DB 테이블에 추가하는 사용자함수
    '''    
    global ent1, ent2, ent3, ent4, dataView # 전역변수로 사용 선언

    ## 엔트리위젯에 학생정보데이터 변수 할당
    ...
    try:
        conn.begin() # BEGIN_TRANSACTION 트랜잭션 시작
        # 쿼리 작성
    ...
    showDatas() # DB테이블의 모든 데이터 조회해서 트리뷰에 표시    
...
def modData():
    ...
    try:
        conn.begin() # BEGIN_TRANSACTION 트랜잭션 시작
        # 쿼리 작성
        query = 'update students set std_name=%s, std_mobile_%s,std_regyear=%s where std_id=%s'
        ...
        ent1.config(state='normal')
        ent1.delete(0, END) # 학생번호 기존 데이터 삭제
        ent1.config(state='readonly') # 학생번호 기존 데이터 삭제
        ent2.delete(0, END) # 학생명 기존 데이터 삭제
        ent3.delete(0, END) # 핸드폰 기존 데이터 삭제
        ent4.delete(0, END) # 입학년도 기존 데이터 삭제
        ent2.focus_set() # 학생명 포커스
...
# 트리뷰(행과 열로 만들어진 데이터 표현할 때 좋은 위젯중 하나)
